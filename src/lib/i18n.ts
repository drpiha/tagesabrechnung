export type Locale = "tr" | "de";
export type Currency = "EUR" | "USD" | "TRY";

export const CURRENCY_SYMBOL: Record<Currency, string> = {
  EUR: "€",
  USD: "$",
  TRY: "₺",
};

export const dict = {
  tr: {
    appName: "Günlük Kasa",
    tagline: "Günlük Kasa Mutabakatı",
    login: "Giriş",
    signup: "Kayıt Ol",
    logout: "Çıkış",
    email: "E-posta",
    password: "Parola",
    name: "Ad Soyad",
    company: "Firma Adı",
    verkaufsort: "Satış Yeri",
    submit: "Gönder",
    dashboard: "Panel",
    history: "Geçmiş",
    reports: "Raporlar",
    settings: "Ayarlar",
    today: "Bugün",
    date: "Tarih",
    time: "Saat",
    tagesumsatz: "Günlük Ciro",
    anfangsbestand: "Başlangıç Tutarı",
    muenzenZielwert: "Hedef Bozuk Para",
    ausgaben: "Giderler",
    ausgabenItem: "Gider",
    addAusgabe: "Gider ekle",
    ausgabeLabel: "Açıklama",
    ausgabeAmount: "Tutar",
    sum: "Toplam",
    totalKassenbestand: "Toplam Kasa Bakiyesi",
    zwischensumme: "Ara Toplam",
    einlage: "Yatırılan",
    uebertrag: "Sonraki güne devir",
    calculate: "Hesapla",
    save: "Kaydet",
    saved: "Kaydedildi",
    delete: "Sil",
    deleted: "Silindi",
    cancel: "İptal",
    edit: "Düzenle",
    total: "Toplam",
    totalCash: "Toplam Nakit",
    banknotes: "Banknotlar",
    coins: "Madeni Paralar",
    summary: "Mali Özet",
    gesamtI: "Banknot Toplamı",
    gesamtII: "Madeni Para Toplamı",
    kassenbestand: "Gün Sonu Kasa Bakiyesi",
    totalEinnahmen: "Toplam Gelir (Banknot + Madeni)",
    stueck: "Adet",
    notes: "Not",
    download: "İndir",
    exportPdf: "PDF İndir",
    exportXlsx: "Excel İndir",
    weeklyTotal: "Haftalık Toplam",
    monthlyTotal: "Aylık Toplam",
    averageDaily: "Günlük Ortalama",
    bestDay: "En Yüksek Gün",
    worstDay: "En Düşük Gün",
    revenueTrend: "Ciro Trendi",
    noData: "Henüz veri yok",
    backToDashboard: "Panele Dön",
    createAccount: "Hesap Oluştur",
    haveAccount: "Hesabınız var mı?",
    noAccount: "Hesabınız yok mu?",
    invalidLogin: "E-posta veya parola hatalı",
    requiredField: "Bu alan zorunludur",
    passwordMin: "Parola en az 8 karakter olmalı",
    emailInUse: "Bu e-posta zaten kayıtlı",
    confirmDelete: "Bu kaydı silmek istediğinize emin misiniz?",
    welcome: "Hoş geldiniz",
    language: "Dil",
    currency: "Para Birimi",
    profile: "Profil",
    changePassword: "Parolayı Değiştir",
    currentPassword: "Mevcut Parola",
    newPassword: "Yeni Parola",
    save_settings: "Ayarları Kaydet",
    addNew: "Günlük Mutabakat",
    print: "Yazdır",
    totalRecords: "Toplam Kayıt",
    period: "Dönem",
    last7: "Son 7 Gün",
    last30: "Son 30 Gün",
    last90: "Son 90 Gün",
    thisMonth: "Bu Ay",
    allTime: "Tüm Zamanlar",
    enterValues: "Tutarları girin ve hesaplayın.",
    bill: "Banknot",
    coin: "Madeni",
    control: "Kontrol",
    update: "Güncelle",
    loadedFromDb: "Bu tarihe ait kayıt yüklendi",
  },
  de: {
    appName: "Tagesabrechnung",
    tagline: "Tägliche Kassenabrechnung",
    login: "Anmelden",
    signup: "Registrieren",
    logout: "Abmelden",
    email: "E-Mail",
    password: "Passwort",
    name: "Name",
    company: "Firma",
    verkaufsort: "Verkaufsort",
    submit: "Senden",
    dashboard: "Übersicht",
    history: "Verlauf",
    reports: "Berichte",
    settings: "Einstellungen",
    today: "Heute",
    date: "Datum",
    time: "Uhrzeit",
    tagesumsatz: "Tagesumsatz",
    anfangsbestand: "Anfangsbestand",
    muenzenZielwert: "Münzen-Zielwert",
    ausgaben: "Ausgaben",
    ausgabenItem: "Ausgabe",
    addAusgabe: "Ausgabe hinzufügen",
    ausgabeLabel: "Bezeichnung",
    ausgabeAmount: "Betrag",
    sum: "Summe",
    totalKassenbestand: "Total Kassenbestand",
    zwischensumme: "Zwischensumme",
    einlage: "Einlage",
    uebertrag: "Übertrag zum nächsten Tag",
    calculate: "Berechnen",
    save: "Speichern",
    saved: "Gespeichert",
    delete: "Löschen",
    deleted: "Gelöscht",
    cancel: "Abbrechen",
    edit: "Bearbeiten",
    total: "Summe",
    totalCash: "Gesamtsumme",
    banknotes: "Banknoten",
    coins: "Münzen",
    summary: "Finanzielle Zusammenfassung",
    gesamtI: "Gesamt I (Banknoten)",
    gesamtII: "Gesamt II (Münzen)",
    kassenbestand: "Kassenbestand bei Geschäftsabschluss",
    totalEinnahmen: "Total Einnahmen (I+II)",
    stueck: "Stück",
    notes: "Notiz",
    download: "Download",
    exportPdf: "PDF herunterladen",
    exportXlsx: "Excel herunterladen",
    weeklyTotal: "Wochenumsatz",
    monthlyTotal: "Monatsumsatz",
    averageDaily: "Tagesdurchschnitt",
    bestDay: "Bester Tag",
    worstDay: "Schwächster Tag",
    revenueTrend: "Umsatzverlauf",
    noData: "Noch keine Daten",
    backToDashboard: "Zurück zur Übersicht",
    createAccount: "Konto erstellen",
    haveAccount: "Bereits ein Konto?",
    noAccount: "Noch kein Konto?",
    invalidLogin: "E-Mail oder Passwort falsch",
    requiredField: "Pflichtfeld",
    passwordMin: "Passwort muss mindestens 8 Zeichen haben",
    emailInUse: "Diese E-Mail ist bereits registriert",
    confirmDelete: "Diesen Eintrag wirklich löschen?",
    welcome: "Willkommen",
    language: "Sprache",
    currency: "Währung",
    profile: "Profil",
    changePassword: "Passwort ändern",
    currentPassword: "Aktuelles Passwort",
    newPassword: "Neues Passwort",
    save_settings: "Einstellungen speichern",
    addNew: "Tagesabrechnung",
    print: "Drucken",
    totalRecords: "Einträge gesamt",
    period: "Zeitraum",
    last7: "Letzte 7 Tage",
    last30: "Letzte 30 Tage",
    last90: "Letzte 90 Tage",
    thisMonth: "Aktueller Monat",
    allTime: "Gesamt",
    enterValues: "Beträge eingeben und berechnen.",
    bill: "Schein",
    coin: "Münze",
    control: "Kontrolle",
    update: "Aktualisieren",
    loadedFromDb: "Eintrag für dieses Datum geladen",
  },
} as const;

export type TKey = keyof typeof dict.tr;

export function t(locale: Locale, key: TKey): string {
  return dict[locale]?.[key] ?? dict.de[key];
}

/** Cent integer → "1.234,56" or "1,234.56" depending on locale, plus currency symbol. */
export function formatMoney(cent: number, currency: Currency, locale: Locale): string {
  const sign = cent < 0 ? "-" : "";
  const abs = Math.abs(cent);
  const whole = Math.floor(abs / 100);
  const frac = abs % 100;

  const useDot = locale === "de" || currency === "EUR" || currency === "TRY";
  const groupSep = useDot ? "." : ",";
  const decSep = useDot ? "," : ".";

  const wholeStr = whole.toString().replace(/\B(?=(\d{3})+(?!\d))/g, groupSep);
  const num = sign + wholeStr + decSep + frac.toString().padStart(2, "0");

  const sym = CURRENCY_SYMBOL[currency];
  // Symbol position: USD prefix, EUR/TRY suffix
  return currency === "USD" ? `${sym}${num}` : `${num} ${sym}`;
}

/** Format a denomination value (e.g. 5000 cent → "50,00 €"). */
export function formatDenom(cent: number, currency: Currency, locale: Locale): string {
  return formatMoney(cent, currency, locale);
}
